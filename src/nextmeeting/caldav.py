...
class CalDavMeetingFetcher:
    ...
    def fetch_meetings(self, args) -> list[object]:
        ...
        if fetch_errors and (
            ...
            for error in fetch_errors:
                print(f"[WARNING] Calendar fetch error: {error}", file=sys.stderr)
    ...
    def _get_calendar_display_name(self, calendar) -> str:
        ...
        if url_obj:
            url_str = str(url_obj.to_python() if hasattr(url_obj, "to_python") else url_obj)
    ...
    def _resolve_calendars(self, client: "DAVClient", args) -> list[tuple[object, str]]:
        ...
        if errors and (getattr(args, "verbose", False) or getattr(args, "debug", False)):
            for error in errors:
    ...
    def _retry_date_search_for_calendar(
        ...
        """
        Retry date search for a specific calendar object.
        Simpler than _retry_date_search since we already have the calendar.
        """
        if not caldav_error:
            raise RuntimeError(f"Failed to query calendar: {original_exc}") from original_exc

        # Try direct query on the calendar object one more time
        try:
            return calendar.date_search(*window)
        except Exception:  # noqa: BLE001
            # If retry fails, re-raise original exception
            raise RuntimeError(
                f"Failed to query calendar after retry: {original_exc}"
            ) from original_exc
